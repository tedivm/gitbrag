...
The FastAPI application is defined in `gitbrag/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `gitbrag/static/` via the `/static/` endpoint
...
Create a new route in `gitbrag/www.py`:
...
Static files are served from `gitbrag/static/`:
...
uvicorn gitbrag.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn gitbrag.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn gitbrag.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
