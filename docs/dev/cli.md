...
```toml
[project.scripts]
gitbrag = "gitbrag.cli:app"
```

After installation, the CLI is available as the `gitbrag` command.
...
```bash
gitbrag my-command --option value argument
```
...
Add a basic command to `gitbrag/cli.py`:
...
```bash
gitbrag process input.txt --output output.txt --verbose
gitbrag process input.txt -o output.txt -v
```
...
```bash
gitbrag export --count 50 --format yaml --output data.yaml
```
...
```python
from gitbrag.cli import syncify
import httpx
...
```python
from gitbrag.services.db import get_engine
...
gitbrag db migrate
gitbrag db seed
gitbrag user create john john@example.com
gitbrag user list
```
...
```python
from gitbrag.commands import database, users
...
def test_version_command_runs():
    ...
    assert "gitbrag" in result.stdout.lower()
