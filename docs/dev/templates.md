...
Templates are stored in the `gitbrag/templates/` directory:
...
The Jinja2 environment is configured in `gitbrag/services/jinja.py`:
...
env = Environment(
    loader=PackageLoader("gitbrag", "templates"),
...
app = FastAPI()
templates = Jinja2Templates(directory="gitbrag/templates")
...
env = Environment(loader=PackageLoader("gitbrag", "templates"))
...
```python
# gitbrag/services/jinja.py
from gitbrag.conf import settings
...
```python
from gitbrag.services.jinja import env
...
```python
from gitbrag.celery import celery
from gitbrag.services.jinja import env
...
from fastapi.templating import Jinja2Templates
from gitbrag.services.jinja import env, response_templates
