...
pytest --cov=./gitbrag --cov-report=term-missing tests

# Generate HTML coverage report
pytest --cov=./gitbrag --cov-report=html tests
# Open htmlcov/index.html in your browser

# Show coverage for specific file
pytest --cov=./gitbrag/services/cache.py tests/services/test_cache.py
```
...
def test_version_command():
    ...
    assert "gitbrag" in result.stdout
```
...
```python
@pytest.mark.asyncio
@patch('gitbrag.services.cache.get_cached')
...
async def test_cache_set_and_get():
    ...
    from gitbrag.services.cache import set_cached, get_cached
...
def test_settings_load():
    ...
    assert settings.project_name == "gitbrag"
...
async def test_with_clean_cache():
    ...
    from gitbrag.services.cache import clear_cache
...
pytest --cov=./gitbrag --cov-fail-under=80 tests
```
