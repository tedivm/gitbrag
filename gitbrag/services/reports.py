...
async def get_or_generate_report(
    ...
    Args:
        github_client: GitHub client (None if unauthenticated)
        username: GitHub username for report
        period: Normalized period name
        show_star_increase: Whether to include star increase
        force_refresh: Force regeneration regardless of cache age
        stale_age: Age in seconds when cache is considered stale
        authenticated_user: Username of authenticated user (None if unauth)
    ...
    if cached_data and cached_meta and not force_refresh:
        cache_age = datetime.now().timestamp() - cached_meta["created_at"]
        is_stale = cache_age >= stale_age

        # Auto-refresh stale cache if authenticated
        if is_stale and github_client:
            should_regenerate = True
            logger.info(f"Cache is stale ({cache_age:.0f}s old), regenerating")
        elif not is_stale:
            logger.info(f"Using fresh cache ({cache_age:.0f}s old)")
            return cached_data, cached_meta
        else:
            # Cache is stale but user not authenticated - return stale cache
            logger.info(f"Using stale cache ({cache_age:.0f}s old) - user not authenticated")
            return cached_data, cached_meta

    # If no cache and no auth, return None
    if not cached_data and not github_client:
        logger.info("No cache available and user not authenticated")
        return {}, {}

    # Regenerate if needed
    if should_regenerate or not cached_data:
        if not github_client:
            # Unauthenticated users can't regenerate - return stale cache if available
            return cached_data or {}, cached_meta or {}
        ...
        metadata = {
            ...
            "created_by": authenticated_user or "system",
            "since": since.isoformat(),
            "until": until.isoformat(),
        }
        ...
        logger.info(f"Cached report for {username} with key {cache_key}")
